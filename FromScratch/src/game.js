...
import meshUrl from "../assets/models/skiing_lady.glb";
import mountainUrl from "../assets/models/mount_timpanogos_early_2017.glb";
import roadTextureUrl from "../assets/textures/6_snow texture-seamless_x2.jpg";

import musicUrl from "../assets/musics/shooting_stars.mp3";
import hitSoundUrl from "../assets/sounds/344033__reitanna__cute-impact.wav";

import obstacle1Url from "../assets/glb/bush.glb";
import tree1Url from "../assets/glb/tree.glb";
...
class Game {
    ...
    update(delta) {
        ...
        /*
        if(this.playerBox.intersectsMesh(this.tracks[0], false)){
        ...
        for (let i = 1; i < this.track.length; i++){
            ...
            let prevTrack = this.tracks[i-1];
            track.rotation.x = 0.0;
            //prevTrack.rotation.x = 0.0;
        }*/
        for (let i = 0; i < this.tracks.length; i++) {
            ...
            if (track.position.z <= 0) {
                ...
                track.position.y = this.tracks[nextTrackIdx].position.y - (TRACK_HEIGHT /*+ Math.random() * 0.5*/);
        ...
        for (let i = 0; i < this.tracks.length; i++) {
            ...
            if(track.position.y < 0){
                track.rotation.x = 0.033333;
            }
            track.position.z -= SPEED_Z / 3 * delta;
            // this.player.position.y = track.position.y + 1.7 / 2;
        }

        // set the player at the floor level
        // this.player.position.y = this.tracks[0].position.y + 1.7 / 2;
    ...
    async createScene() {
        ...
        res.animationGroups[0].play(true);
